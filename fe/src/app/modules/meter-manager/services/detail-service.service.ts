...
export class DetailServiceService {
  ...
  getDetailsByMeterId(meterId: number): Observable<DetailData[]> {
    return this.http.get<{predictions: any[], total_count: number, message: string}>(`${this.apiUrl}/api/predictions/predictions?meter_id=${meterId}&limit=50`).pipe(
      map(response => this.transform(response.predictions)),
      catchError(error => {
        console.error('Error fetching predictions:', error);
        return this.getMockDetailsByMeterId(meterId);
      })
    );
  }

  private transform(predictions: any[]): DetailData[] {
    return predictions.map((pred, index) => ({
      id: pred.p_id || index + 1,
      name: `Đồng hồ ${pred.meter_id}`,
      timeStamp: new Date(pred.prediction_time),
      flow: pred.recorded_instant_flow || 0,
      prediction: pred.predicted_label === 'Rò rỉ' ? 1 : 0,
      predConfidence: pred.confidence || 0
    }));
  }

  private getMockDetailsByMeterId(meterId: number): Observable<DetailData[]> {
